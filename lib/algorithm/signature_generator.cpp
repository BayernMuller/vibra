...
SignatureGenerator::SignatureGenerator()
    : mInputPendingProcessing(), mSampleProcessed(0), mMaxTimeSeconds(3.1),
      mNextSignature(16000, 0), mRingBufferOfSamples(2048, 0),
      mFFTOutputs(256, fft::RealArray(1025, 0.0)), mSpreadFFTsOutput(256, fft::RealArray(1025, 0.0))
{
}
...
void SignatureGenerator::doFFT(const LowQualityTrack &input)
{
    ...
    std::copy(mRingBufferOfSamples.begin() + mRingBufferOfSamples.Position(),
              mRingBufferOfSamples.end(), excerpt_from_ring_buffer.begin());

    std::copy(mRingBufferOfSamples.begin(),
              mRingBufferOfSamples.begin() + mRingBufferOfSamples.Position(),
    ...
    for (auto position = 0u; position < 1025; ++position)
    {
        ...
        for (auto former_fft_num : {-1, -3, -6})
        {
            auto &former_fft_ouput =
                mSpreadFFTsOutput[(mSpreadFFTsOutput.Position() + former_fft_num) %
                                  mSpreadFFTsOutput.Size()];
...
void SignatureGenerator::doPeakRecognition()
{
    ...
    const auto &fft_minus_49 =
        mSpreadFFTsOutput[(mSpreadFFTsOutput.Position() - 49) % mSpreadFFTsOutput.Size()];
